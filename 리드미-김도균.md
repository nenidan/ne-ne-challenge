...
카카오/네이버 간편 로그인 → 3초 만에 시작하는 챌린지


💳 **토스페이먼츠 포인트 충전**<br>

실제 돈 → 포인트 안전 변환 → 선불 충전으로 과소비 방지
...
[//]: # ()
[//]: # (토스페이먼츠로 안전한 결제 : 필요한 만큼만 충전하는 스마트한 시작)
...
[//]: # ()
[//]: # (All Or Nothing 시스템 : 성공하면 상금 획득, 실패하면 전액 손실)
...
[//]: # (🛍️ **보상은 실속있게!**<br>)
[//]: # ()
[//]: # (챌린지 상금으로 실물 상품 구매 : 노력이 진짜 선물이 되는 순간]&#40;&#41;)
...
### 💳 결제
- **토스페이먼츠 연동**: 토스 페이먼츠 API를 연동한 포인트 결제 기능
- **서킷브레이커 적용**: Resilience4j로 토스 장애 격리
- **Spring Event 비동기**: 결제 승인 후 포인트 충전 분리 처리로 응답 속도 개선 및 장애 격리

<br>

### 💰 포인트
- **포인트 사용**: *FIFO 알고리즘을 활용한 포인트 차감 정책*
- **충전 재시도 메커니즘**: @Retryable로 충전 실패 시 3회 자동 재시도 (1초 간격)
...
|               **FCM 알림, Redis ZSet 재전송 큐**<br/>병렬 처리 최적화                |            **Promtail & Loki, Grafana 대시보드**<br/>시스템 전반 모니터링            |               **Elasticsearch 검색 기능 향상**<br>포인트 조회 캐싱               |
