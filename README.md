...
작심삼일, 누구나 겪는 이야기죠.  
우리는 이 <font size="6">__작심삼일의 벽__</font>을 깨트리는 **강력한 동기부여 플랫폼**을 만들었습니다.

💰 **<font size="6">__참가비__</font>를 걸고 함께 도전하세요!**
...
- 권한별 기능 접근 제어
- 캐싱 및 스케줄러 기반 성능 최적화
- Elasticsearch 기반 통합 검색 (이메일, 닉네임, 자기소개)


<h3>🎯 챌린지 도메인</h3>
- Redisson 분산 락을 통한 동시성 제어
- 4단계 챌린지 라이프사이클 자동 관리
- 일일 인증 기록 및 중복 인증 방지
- 인증 성공률 기반 보상 자동 분배
- 참가비 환불 정책 자동 적용 (진행 전 전액, 중도 포기 시 비환급)


<h3>💳 결제 도메인</h3>
- 결제 전 orderId/금액 선저장
- 포인트 자동 충전 및 실패 시 롤백 처리
- 커서 기반 대용량 결제 내역 조회


<h3>💰 포인트 도메인</h3>
- FIFO 방식 포인트 차감
- 챌린지, 환불, 결제 취소 연동 포인트 자동 회수
- 대용량 거래 내역 커서 기반 조회


<h3>🛍️ 상점 도메인</h3>
- 상품 등록/삭제 시 캐시 재적재 및 Elasticsearch 동기화 (CQRS 패턴)
- 재고 TCC 패턴 적용 (예비/실제 재고 분리)
- 주문 생성/취소 실패 시 보상 트랜잭션으로 일관성 확보


<h3>🔔 알림 도메인</h3>
- 플랫폼별 FCM 토큰 관리
- Redis ZSet 기반 알림 재전송 시스템 (최대 3회 재시도, 스케줄러 자동 처리)
- 알림 타입별 필터링 및 발송 이력 관리
